...
def lambdaLoss(y_pred, y_true, eps=DEFAULT_EPS, padded_value_indicator=PADDED_Y_VALUE, weighing_scheme=None, k=None, sigma=1., mu=10.,
    ...
    if k is not None:
        ...
        padded_pairs_mask = padded_pairs_mask & ndcg_at_k_mask
...
def lambdaLossRecall(y_pred, y_true, eps=DEFAULT_EPS, padded_value_indicator=PADDED_Y_VALUE, weighing_scheme=None, k=None, sigma=1., mu=10.,
               reduction="sum", reduction_log="binary"):
    """
    LambdaLoss framework for LTR losses implementations, introduced in "The LambdaLoss Framework for Ranking Metric Optimization".
    Contains implementations of different weighing schemes corresponding to e.g. LambdaRank or RankNet.
...
if __name__ == '__main__':
    ...
    # Create random predictions and ground truth labels
    y_pred = torch.rand((batch_size, slate_length)).float()
    y_true = torch.randint(0, 2, (batch_size, slate_length)).float()  # Binary relevance labels

    # Call the lambdaLoss function
    loss = lambdaLoss(y_pred, y_true)
